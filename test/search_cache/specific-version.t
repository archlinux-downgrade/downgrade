...
With a version string included
  ...
  > DOWNGRADE_FROM_CACHE=1 search_packages 'foo' | filter_packages 'foo' '=~' '^1' | sort
  ...
  >   "$cache/foo-completions-1.1-1-any.pkg.tar.gz"
  > write_pacman_conf "CacheDir = $cache/"
  > DOWNGRADE_FROM_CACHE=1 search_packages 'foo' | filter_packages 'foo' '=' '1.1-1' | sort
  /tmp/*/foo-1.1-1-any.pkg.tar.gz (glob)
  ...
  >   "$cache/foo-completions-1.1-1-any.pkg.tar.gz"
  > write_pacman_conf "CacheDir = $cache/"
  > DOWNGRADE_FROM_CACHE=1 search_packages 'foo' | filter_packages 'foo' '>=' '2.0-1' | sort
